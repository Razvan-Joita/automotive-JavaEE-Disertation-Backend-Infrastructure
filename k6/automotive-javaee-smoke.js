...
const BASE_URL = (__ENV.BASE_URL || 'http://localhost:8087').replace(/\/$/, '');
const THINK_TIME_SECONDS = Number(__ENV.THINK_TIME_SECONDS || '1');

export const options = {
  scenarios: {
    automotive_javaee_smoke: {
...
const apiLatency = new Trend('automotive_javaee_api_latency', true);
const apiSuccess = new Rate('automotive_javaee_api_success');
...
export default function () {
  group('read automotive JavaEE API endpoints', () => {
...
export function handleSummary(data) {
  return {
    ...
    '/scripts/results/automotive-javaee-summary.json': JSON.stringify(data, null, 2),
  };
}
...
function textSummary(data) {
  ...
  return [
    'Automotive JavaEE API k6 summary',
    `Target: ${BASE_URL}`,
